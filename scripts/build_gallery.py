...
def update_scenarios_json(results: dict):
    ...
    Additive: scenarios listed in this script's ``SCENARIOS`` dict are
    rebuilt from source and their entries are regenerated here. Any other
    scenario already present in ``scenarios.json`` (e.g. static fixtures
    generated by their own scenario-local ``_generate.py``) is preserved
    as-is. Their contract is still enforced by ``scripts/check_gallery.py``;
    this script just does not rebuild them.
    """
    ...
    # Preserve scenarios not built by this script (static fixtures, externally
    # generated scenarios). Their build_gallery.py rebuild is intentionally a
    # no-op; their contract is enforced via scripts/check_gallery.py.
    for scenario_id, entry in existing.items():
